...
create.circle4 <- function(matrix1,Mw,distance,coordsx,coordsy){
  ...
  for (i in 1:(precision*distance)){
    ...
    if(abs((angle_templong - angle_lineperp1 + pi + 2*pi) %% (2*pi) - pi)< pi/(4*R)){
      matrix1 <- create_line(matrix1=matrix1,Mw = Mw,templong = templong[i],templat = templat[i],coordsx = coordsx,coordsy=coordsy)
    
      }else if(abs((angle_templong - angle_lineperp2 + pi + 2*pi) %% (2*pi) - pi) < pi/(4*R)){
      matrix1 <- create_line(matrix1=matrix1,Mw = Mw,templong = templong[i],templat = templat[i],coordsx = coordsx,coordsy=coordsy)
    }
...
#dissipate <- function(Mw,degpersqr){
  ...
  #print(dist)
  #print(dist/degpersqr/110)
  distance <- round_any(dist/degpersqr/110,1)
  print('distance')
  print(distance)
  print(Mw_dis)
  #distance <- round_any(Mw_dis/degpersqr,1)
  #*****************************************
  diss_per_sqr <- degpersqr
  #rm(tempMw)
  #iterate back from the largest distance and smallest Mw
  #print(distance)
  if(distance >0 ){
    for (i in seq(1,distance,by=1)){
      ...
      #print(kmdist)
      #print(log10(2 * pi * kmdist^2.4)/1.5)
...
dissipate_FL_Mw <- function(Population_Data,coordsx,coordsy,Mw){
  ...
  coordsy <- round_any((coordsy - latdim[1])/degpersqr,1)
...
raster1 <- raster(matrix)
raster2 <- as.data.frame(raster1)

Mw33 <- asc.from.raster(raster1)
...
graph2 <- ggplot(aes(x = s1, y = s2),data = TRy50) + geom_raster(aes(fill = z)) + scale_fill_continuous(low = 'green', high = 'red')
#graph
graph2
...
graph <- ggplot(data = raster1) + geom_raster() + scale_color_continuous(name = 'Mw', low = 'green', high = 'red')
# plot(raster(matrix))
coordsx2 <- coordsx
coordsy2 <- coordsy
coordsy
